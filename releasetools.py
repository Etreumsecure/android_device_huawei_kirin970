...
"""Custom OTA commands for kirin970"""
...
#	time.sleep(2)
  ...
  info.output_zip.write(os.path.join(TARGET_DIR, "device/huawei/kirin970/recovery/data-formatter.sh"), "install/bin/data-formatter.sh")
#  info.output_zip.write(os.path.join(TARGET_DIR, "device/huawei/kirin970/recovery/finalize.sh"), "install/bin/finalize.sh")
#  info.output_zip.write(os.path.join(TARGET_DIR, "device/huawei/kirin970/recovery/stock-check.sh"), "install/bin/stock-check.sh")
...
#  info.script.AppendExtra('assert(run_program("/sbin/chmod","755", "/tmp/install/bin/stock-check.sh") == 0 || abort("Could not chmod stock-check"););')
 ...
 # info.script.AppendExtra('assert(run_program("/tmp/install/bin/stock-check.sh") == 0 || abort("stock-check failed."););')
  info.script.AppendExtra('assert(run_program("/tmp/install/bin/data-formatter.sh") == 0 || abort("ERROR!: Data could not be formatted!"););')
